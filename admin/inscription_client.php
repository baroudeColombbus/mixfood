    ...
    if (empty($message)) {// si la variable est vide, c'est que tu n'a pas d'erreur
        $utilisateur = executeRequete(" SELECT * FROM utilisateur WHERE email = :email OR telephone = :telephone" ,
                            ...
                            if ($utilisateur->rowCount() > 0) { // si la requête retourne des lignes c'est que l'adresse mail existe déjà
                                ...
                                $succes = executeRequete(" INSERT INTO utilisateur (nom,prenom, email, telephone, mot_de_passe,adresse, ville, code_postal,date_enregistrement,statut_utilisateur) VALUES (:nom, :prenom, :email, :telephone, :mot_de_passe, :adresse, :ville, :code_postal, now(), 'client')", 
                                    array(
                                        ':nom'              => $_POST['nom'],
                                        ':prenom'           => $_POST['prenom'],
                                        ':email'            => $_POST['email'],
